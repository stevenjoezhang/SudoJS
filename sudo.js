//https://github.com/haoxiaobo/SudoJS
//https://www.zhihu.com/question/22043229/answer/44315364
//https://segmentfault.com/a/1190000004995017
...
Cell.prototype.toString = function () {
	if (this.N == null || this.N == undefined) return "";
...
function Sudo(width, height, canvasGame) {
	...
	this.checkValid = function (sudo, i, j) {
		var row = {}, col = {}, subSudo = {};
		for (var k = 0; k < 9; k++) {
			var cur1 = sudo[i][k].N, cur2 = sudo[k][j].N;
			if (cur1) {
			...
			var key = sudo[Math.floor(i / 3) * 3 + Math.floor(k / 3)][Math.floor(j / 3) * 3 + Math.floor(k % 3)].N;
	...
	this.findAnswer = function (sudo) {
		var stack = [], flag = false, time = new Date().getTime();
		for (var i = 0; i < 9; i++) {
			for (var j = 0; j < 9; ) {
				if (new Date().getTime() - time >= 1000) return 1; // 数独可能无解的超时处理
				if (sudo[i][j].N === 0 || flag) {
					flag = false;
					var k = sudo[i][j].N + 1;
					while (k < 10) {
						sudo[i][j].N = k;
						if (this.checkValid(sudo, i, j) == 0) {
							stack.push([i, j++]);
							break;
						}
						k++;
	...
	this.str = [1, 2, 3, 4, 5, 6, 7, 8, 9];

	this.rand = function(array) {
		return array[Math.floor(Math.random() * array.length)];
	};

	this.checkArray = function(sudo, p, q) {
		for (var i = 0; i < 9; i++) {
			if (sudo[p][q] == sudo[i][q] && p != i) return 1;
		}
		for (var j = 0; j < 9; j++) {
			if (sudo[p][q] == sudo[p][j] && q != j) return 1;
		}
		for (var i = p - p % 3; i < p - p % 3 + 3; i++) {
			for (var j = q - q % 3; j < q - q % 3 + 3; j++) {
				if (sudo[p][q] == sudo[i][j] && (p != i || q != j)) return 1;
			}
		}
		return 0;
	};
	...
	this.Init = function (level) {
		...
		do {
			...
			for (var count = 0; count < 11; count++) {
				var i = this.rand(this.str) - 1;
				var j = this.rand(this.str) - 1;
				if (this.Array[i][j] == undefined) {
					do {
						this.Array[i][j] = this.rand(this.str);
					}
					while(this.checkArray(this.Array, i, j));
				}
				else count--;
			}
			...
			for (var i = 0; i < 9; i++) {
				...
				for (var j = 0; j < 9; j++) {
					this.Cells[i][j] = new Cell(this.Array[i][j] || 0);
				}
			}
		}
		while (this.findAnswer(this.Cells) == 1);
		...
		//this.CanvasGame.addEventListener("touchstart", onTouchStart, false);
		//this.CanvasGame.addEventListener("touchmove", onTouchMove, false);
		this.CanvasGame.addEventListener("touchend", onTouchEnd, false);
	...
	this.Draw = function () {
		...
		for (var i = 0; i < 9; i++) {
			for (var j = 0; j < 9; j++) {
				...
				if (cell.N == null) {
					...
					if (cell.InConflict) ctx2.fillStyle = "red";
					else if (cell.Fixed) ctx2.fillStyle = "#000000";
					else ctx2.fillStyle = "#008800";
		...
		for (var i = 0; i < 10; i++) {
			if (i % 3 == 0) ctx2.lineWidth = 3.0; // 设置线宽
			else ctx2.lineWidth = 1.0; // 设置线宽
		...
		if (this.bUseBuffer) this.ctx2.drawImage(cvBuffer, 0, 0);
	};
...
EditPannel.prototype.GetHitNumber = function (x, y) {
	...
	if (j < 0 || j > 2 || i < 0 || i > 3) return -1; // 范围外
	if (i == 3) return null;

	var n = i * 3 + j + 1;
	if (!InArray(n, this.NList)) return null;
	return n;
};
...
function onTouchEnd(evt) {
	//evt.PerventDefault();
	var loc = getPointOnCanvas(evt.target, evt.targetTouches[0].pageX, evt.targetTouches[0].pageY);
	sudo.ProcMouseClick(loc);
	//var index = sudo.getCellIndex(loc);
}
